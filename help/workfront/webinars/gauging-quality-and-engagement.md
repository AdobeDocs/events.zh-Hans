---
title: 咨询专家 — 评估质量和参与度
description: 了解如何构建可回答质量和参与问题的报告。 该网络研讨会于2019年11月13日录制。
activity: use
...
了解如何构建可回答质量和参与问题的报告。 该网络研讨会于2019年11月13日录制。
...
有一些字段可供过滤，但在您尝试按这些字段分组时不可用。 您是否正在努力使其成为一致的选项？

**回答**

报表工具不允许您按动态字段或可一次选择多个选项的字段进行分组，例如复选框字段。 您只能对具有单个值的字段进行分组，即使它们可能有多种选择。

您可以按复选框进行筛选和查看，只是不能对它们进行分组。

**问题**

在迭代“工作角色”示例中，能否将主要角色显示为一个粗体？

**回答**

在迭代中，我们可以确定主要工作角色。 我们需要在valueexpression中执行此操作，但在valueexpression中无法识别HTML代码。 所以我们需要想出另一种方法来确认这个角色是主要角色。 在此代码中，我将“**”放在主要角色名称之前和之后。 尝试一下，看看你喜欢什么：
...
嗨！ 我正在为我们的编辑团队整理一个工作流，跟踪文章在其生命周期中的位置（初始写入 — >部门审核 — >最终编辑 — >发布）。 他们想要轻松查看它当前所在的“里程碑”或“任务”。 我收到的反馈是标准的“里程碑”视图（使用红色或绿色底纹）太“忙碌、复杂”。 是否可以在表或网格中仅显示“项目名称”和“当前里程碑”？

**回答**

可以。您可以创建一个任务报告，以显示当前正在处理的里程碑任务及其关联的任务。 您可以在表格或列表报告中执行此操作。
...
如果您已创建验证审批报告，则可以使用文本模式将项目信息提取到列中。 例如，如果您想在列中引用项目名称，则可以使用以下内容：
...
要在单个报告中引用项目信息和验证信息，您需要创建验证审批报告。 目前，无法将来自校样的评论提取到此报告中，但是，可以在“审批者决定”列下自己的行项目中找到每个校样审批者决定。

**问题**

我经常使用sharecol来创建单页状态（许多列）。 但是，我发现在创建报告后，如果要在页面顶部添加列，则返回并进行修改会非常耗时。 您是否有任何进行此类更改的提示或技巧？

**回答**

如果您谈论的是将列放在文本模式列列表的顶部，则只需手动为列重新编号。

但是，如果您已创建了报告，其中第一列是一些共享列，并且希望将另一个共享列放在列表顶部，则操作会更轻松。

在报表编辑器中，只需添加几个新列并将它们拖动到“列（视图）”屏幕的最左侧。 执行此操作后，请查看以前位于左列的内容，您会发现文本模式的列号都已递增。 根据需要将任意数量的空白列拖动到那里。 保存前，请确保将某些内容放入这些空白列中，否则这些内容将被删除。

**问题**

您好 — 关于最终错误报告 — 如何为多个项目完成报告 — 如果多个项目中出现错误??

**回答**

您可以按项目组合或项目进行过滤，具体取决于您组织工作的方式。

您还可以根据请求队列进行过滤。 您可能需要为每个项目设置请求队列，您可以将客户用户创建为审阅人，审阅人可以直接登录请求队列并将其提交到您与他们共享的请求队列。

**问题**

第一个报告基于项目/项目名称，这是否也基于任务完成，如果是，那么将它们分组的最佳方式是什么，任务名称可能更多样不同……谢谢！

**回答**

所有这些报告可以作为任务、问题或项目报告来完成，具体取决于您决定如何跟踪这些内容。

对任务进行分组的常见方法是首先按其项目名称对任务进行分组，然后按每个项目中的任务名称对任务进行分组。 这样，如果您有两个同名的任务，就很容易看到它们所在的项目。

**问题**

我想知道哪些校样是优秀的，它们与什么任务和项目相关联，何时路由，何时到期，以及谁是审查方和批准方。

**回答**

未完成的校样可以通过等待决策>等于> True在校样审批报告中进行过滤。 有一个“审批者”列，它将告知您尚未做出决策的人员。

您可以使用文本模式引用校样绑定的任务或项目（请参阅以下示例）。
...
至于路由日期、截止日期和审查方，这些字段目前无法拉入任何Workfront的报表，因此您需要直接单击验证以查看该信息。

**问题**

能否设置自定义表单，以便在请求者完成请求后自动发送给请求者？ 比如“客户满意度”调查？

**回答**

有一件事你可以做，也许能满足你的需要。 您可以向问题附加提醒通知，在输入了实际完成日期后，该问题将向输入者发送电子邮件。 输入者是创建问题的人员。

您可以创建提醒通知，就像我们在“提醒填写行动后复查(AAR)”的网络研讨会中所做的那样，只不过这会是一个问题提醒。 您可能还希望为其创建一个电子邮件模板，以提供调查链接。 您必须手动将提醒通知应用于每个问题（或使用批量编辑）。

集成会更好，因为它可以自动执行手动步骤，但提醒通知可以立即完成。

**问题**

我创建了一个报告，其中按模板类型显示了项目。 我可以列出项目所有者，但不能列出分配给项目的人员。

**回答**

如果要将“项目团队”（“人员配备”选项卡）拉入项目报告中的列，则需要通过文本模式创建此列。 文本模式如下：
