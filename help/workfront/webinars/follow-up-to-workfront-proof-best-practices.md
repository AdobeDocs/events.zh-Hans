...
不過，您可以注意它只會與擁有校訂授權的群組成員共用。 您將無法與沒有校訂授權的Workfront使用者或來賓共用範本。

**问题**

如果拒絕校訂，如何將校訂重新路由到校訂所有者？

**回答**

校樣擁有者會在所有校樣階段中停留在校樣中。 如果校訂被拒絕，校訂本身不需要被路由回所有者。 相反地，校樣所有者將透過電子郵件收到所作決定的通知、檢閱評論，並開始使用新版本。
...
新增校訂時，您需要捲動到底部直到到達「校訂設定」。 在那裡，您會看到「下載原始檔案」的核取方塊，您可以選取或取消選取？
...
對於「收件者可以檢視所有版本」的共用設定 — 如果設定為「已停用」，如果收件者不在版本1上但位於版本2，則無法比較版本1和2。 請注意，無論設定為何，擁有監督員或管理員校訂許可權層級的Workfront使用者將能夠檢視所有版本。

**问题**

我們可以請幾個人上傳新版本嗎？ 例如，撰稿人上傳版本1，我們便在階段1中有提供者。 他們看到需要進行的變更，可以上傳版本2嗎？

**回答**

您可以讓校樣收件者在符合下列條件時建立新版本的校樣： 1)他們是校樣的所有者 — 或2)他們在校樣上設定為作者或版主校樣角色 — 或3)他們設定為具有主管或管理員的校樣許可權等級。

**问题**

如何處理多個校訂(例如 A、B和C)搭配自動化工作流程。 您會重新開始嗎？

**回答**

建立初始版本的校訂時，您可以將自動化工作流程範本套用至多個校訂。 若要這麼做，請遵循以下路徑：檔案>新增>校訂。 在新校訂頁面上，選取要上傳的多個檔案、套用自動化工作流程範本並建立校訂。
...
您可以將校樣上的列印摘要匯出到PDF檔案。 這將包含所有評論、標示、回覆和決定。
...
若要檢視現有校訂上的校訂設定，您將想要遵循以下路徑：檔案標籤>選取校訂>按一下「校訂詳細資訊」>從開啟的校訂詳細資訊視窗中，展開「設定」區域。
...
如果您是私人階段內的校訂收件者，將能夠標籤該私人階段內的任何人。 如果您未處於私人階段，您將無法從私人階段中標籤某人。
...
您無法將作用中的階段停用，但您可以「鎖定」階段，以防止階段內的人員發表評論和做出決定。

**问题**

當舞台中1位或更多稽核者表示需要變更時，會在幕後發生什麼情況？ 誰會收到上傳新版本的通知？

**回答**

這將取決於校訂建立者和/或校訂收件者校訂上的「電子郵件警報」設定。 我建議校訂建立者/擁有者設定有「決定」的電子郵件警報，以便在對校訂做出決定時透過電子郵件通知他們。
