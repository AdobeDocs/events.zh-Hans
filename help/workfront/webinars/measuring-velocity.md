...
當您在報告中依任何專案分組時，可讓您建立圖表。 然後，您可以在「圖表」標籤中設定X或Y軸。
...
在文字模式中可以使用值運算式來執行這些計算。 雖然我們無法完成首次持續時間或首次認可日期，但我們需要將這些資料擷取到不會變更的地方。
...
報表會顯示您要檢視的目前設定為預設之基準線欄位，因此您可以變更基準線並重新整理報表，以檢視具有新基準線的欄位。
