---
title: Nathan的五个方面 — Workfront世界的基本API
description: 探索Workfront API及其用例和集成方法以增强工作流。 了解自动化和数据管理的实用技巧。
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3435
last-substantial-update: 2026-06-24T00:00:00Z
jira: KT-21710
source-git-commit: b4b48b83a52b48c67d0d514600045b138ed24f36
workflow-type: tm+mt
source-wordcount: '222'
ht-degree: 1%

---


# Nathan的五个方面 — Workfront世界的基本API

好奇Workfront API真正可以为您做什么？ 加入Adobe技术支持工程师三世的Nathan Johnson，他将分解基础知识并将API引入现实世界。

在此会议中，Nathan先介绍了Workfront API的基础知识、它是什么、它的工作方式以及它为什么重要，然后再深入探讨他最喜欢的五个用例，这些用例展示了其实际操作的强大功能。

>[!VIDEO](https://video.tv.adobe.com/v/3492587/?learn=on&enablevpops)

从基础知识到实际应用程序，本会议将在每天与其Workfront API合作的专家的指导下，为您提供一条了解API世界的清晰、易懂的途径。 无论您是初次使用API，还是希望扩展您的技术工具包，此会话都将帮助您在概念与功能之间建立起联系。

正在寻找活动期间共享的更多资源？ 查看[Experience League社区帖子](https://experienceleaguecommunities.adobe.com/adobe-workfront-23/event-follow-up-nathan-s-five-faves-basic-apis-for-the-workfront-world-june-23-2026-251328)！

每月都会添加新活动，因此请查看[Experience League活动页面](https://experienceleague.adobe.com/en/events)以了解最新会话。
