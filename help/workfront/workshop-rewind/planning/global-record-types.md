---
title: Workfront计划全局记录类型说明 — 何时、为什么以及如何使用它们
description: 了解何时、为什么以及如何使用Workfront的全局记录类型更好地规划和简化工作流。
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3378
last-substantial-update: 2026-06-02T00:00:00Z
jira: KT-21434
exl-id: 69bc7e07-60e2-4c32-9e8d-e5c3c78b65a8
source-git-commit: 168e08b888ba6f610da50394d78575878fc8ac8b
workflow-type: tm+mt
source-wordcount: '157'
ht-degree: 0%

---

# Workfront计划全局记录类型说明 — 何时、为什么以及如何使用它们

与高级产品经理Lilit Mkrtchyan一起，深入了解Workfront规划中最具影响力的创新之一 — 全球记录类型。 该会议介绍全局记录类型如何让您在支持各个团队独特需求的同时，跨工作区标准化关键数据结构。

>[!VIDEO](https://video.tv.adobe.com/v/3491591/?learn=on&enablevpops)

正在寻找活动期间共享的更多资源？ 查看[Experience League社区帖子](https://experienceleaguecommunities.adobe.com/adobe-workfront-23/event-follow-up-workfront-planning-global-record-types-explained-when-why-and-how-to-use-them-may-28-2026-250827)！

每月都会添加新活动，因此请查看[Experience League活动页面](https://experienceleague.adobe.com/en/events?lang=en)以了解最新会话。
