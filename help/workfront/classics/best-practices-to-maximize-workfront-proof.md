...
当请求用户有权访问报告ProofHQ时，这是否还会授予他们访问左侧部分（即工作流、联系人、帐户设置等）的权限？
...
如果另一用户拒绝或批准进行更改，是否可以触发新决策？ 例如，验证部门看到某个内容，项目经理必须做出新决定……即使他们已查看并批准了该内容。 （尝试不让项目经理不必等待校对部门进行审阅）？
...
在哪里可以查看已查看和/或批准验证的每个人的列表？哪一天？在什么时间？ 用于审核等是否还有一处可查看所有验证版本的所有审阅和批准？
