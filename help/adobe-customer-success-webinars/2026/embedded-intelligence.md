---
title: 嵌入式智能转变
description: 探索Adobe Experience Cloud如何引入辅助智能以增强决策并加快insight到行动的周期。
solution: Experience Platform
feature: AI Assistant, Personalization
topic: Artificial Intelligence
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3008
last-substantial-update: 2026-01-14T00:00:00Z
jira: KT-20075
exl-id: 80806f06-8d5a-480c-8b18-6efc622a4c77
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '75'
ht-degree: 0%

---

# 我们之间的代理：嵌入式智能转变

了解人工智能如何在Adobe Experience Cloud中从概念转变为功能。 了解嵌入式智能的发展，并探索可增强分析、内容和客户参与度的早期应用程序。 了解组织如何准备其数据、团队和治理以负责任且高效地采用这些功能。

>[!VIDEO](https://video.tv.adobe.com/v/3479031/?learn=on&enablevpops)
