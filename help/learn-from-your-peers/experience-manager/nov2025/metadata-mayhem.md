---
title: 元数据的混乱局面已成定局
description: 了解关于实施AEM Assets和Workfront连接器、映射元数据和调整角色以实现成功转出的专家提示。
solution: Experience Manager, Workfront, Experience Manager Assets
feature: Metadata
role: Admin, User
level: Beginner, Intermediate
doc-type: Event
duration: 3613
last-substantial-update: 2025-11-14T00:00:00Z
jira: KT-19693
source-git-commit: a2880622b706440262b91279a69eadda493cbe81
workflow-type: tm+mt
source-wordcount: '111'
ht-degree: 0%

---


# 元数据的混乱局面已成定局

在此网络研讨会中，Adobe专家Autumn Earle和John Lemme将分享他们的有效实施AEM Assets和Workfront原生连接器的最佳实践。 您还将获得许多有价值的规划资源，这些资源将帮助您协调利益相关者、优化集成策略并更快地获得价值。

## 您将学习的内容

* 集成实施过程的分步概述
* AEM和Workfront之间的元数据字段映射的最佳实践
* 成功转出所需的关键角色和角色

>[!VIDEO](https://video.tv.adobe.com/v/3476712/?learn=on&enablevpops)
