---
title: 使用Adobe Experience Manager迁移体验并使其现代化，以实现用户参与和AI原生发现
description: 了解Adobe Experience Manager如何使用AI来高效地进行网站现代化和迁移，从而改善参与和发现。
feature: Edge Delivery Services, Release Information
topic: Artificial Intelligence, Integrations, Migration
role: Developer, User
level: Beginner, Intermediate
doc-type: Event
duration: 975
last-substantial-update: 2025-12-11T00:00:00Z
jira: KT-19987
exl-id: 34d729aa-6eb8-48f7-a9ea-66d6f179eb39
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '202'
ht-degree: 0%

---

# 使用Adobe Experience Manager迁移体验并使其现代化，以实现用户参与和AI原生发现

探索Adobe Experience Manager如何为AI时代转变企业网站。 了解AI代理如何自动执行网站迁移、丰富内容并提高可发现性，从而缩短启动时间。 深入了解如何集成对话界面、创建新页面以及增强设计以提高用户参与度。

>[!VIDEO](https://video.tv.adobe.com/v/3477717/?learn=on&enablevpops)

本课程 — 使用Adobe Experience Manager迁移体验并使其现代化，以实现用户参与和AI原生发现 — 重点介绍了Adobe的云可靠性工程师Giorgia Fiscaletti和Adobe的工程总监Paolo Mottadelli。 此演示文稿从巴塞尔现场录制，它展示了Adobe Experience Manager代理如何在AI时代实现企业网站现代化。 了解AI代理如何自动迁移、丰富和标准化内容以进行创新型发现、处理块实施和设计生成，从而缩短启动时间并提高内容可发现性。

特别感谢我们的赞助商Algolia和Ensemble对Adobe Developers Live 2025的支持。

## 后续步骤

* 在[Experience League](https://adobe.ly/47ss08A)上继续对话
* 发现[即将举行的活动](https://developerevents.adobe.com)
