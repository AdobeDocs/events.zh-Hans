...
description: 与Adobe的资深开发人员Raymond Camden一起学习Web组件基础知识，包括自定义元素、影子DOM和HTML模板，以及嵌入PDF和构建可排序表以增强应用程序等实际示例。
...
与Adobe的资深开发人员宣传专员Raymond Camden一起学习Web组件基础知识，包括自定义元素、影子DOM和HTML模板。 探索嵌入PDF和构建可排序表等真实示例，以使用可重用的现代解决方案增强您的应用程序。
...
* **核心技术** Web组件是使用三种核心技术构建的：自定义元素**影子DOM和HTML模板。
* **自定义元素**&#x200B;自定义元素允许创建新的HTML标记。 它们必须使用烤肉串大小写并包含短划线。 JavaScript类用于定义其行为。
...
* 如果HTML加载失败，**渐进式增强功能** Web组件可以增强现有JavaScript而不会中断功能。
* **后续步骤和资源**&#x200B;此演示文稿建议探索表单参与率、声明性影子DOM、自定义HTML属性和服务器端渲染。 资源包括MDN、webcomponents.org以及Ben Farrell撰写的“Web组件正在运行”一书。
